"""
Demo script for running the BacDive single-source processing pipelines.

Usage example:
python demo_bacdive.py \
    --raw_input_dir ../../sample_data/01_bacdive/raw_data \
    --struct_dir= ../../sample_data/01_bacdive/config/structure \
    --output_dir= ../../result/single_source_demo/DB_Bacdive \
    --ref_path= ../../sample_data/00_reference/reference.tsv \
    --csn_path= ../../sample_data/00_reference/csn.tsv
"""
import argparse
from pipelines.run_single_bacdive import run_bacdive_pipeline


def main():
    parser = argparse.ArgumentParser(description="Demo for BacDive single-source pipelines")

    parser.add_argument(
        "--raw_input_dir",
        required=True,
        help="Directory for raw data input"
    )

    parser.add_argument(
        "--struct_dir",
        required=True,
        help="Directory containing structure configuration files"
    )

    parser.add_argument(
        "--output_dir",
        required=True,
        help="Directory for saving processed results"
    )

    parser.add_argument(
        "--ref_path",
        required=True,
        help="Reference for supplementary microbiome information"
    )

    parser.add_argument(
        "--csn_path",
        required=True,
        help="CSN for supplementary microbiome information"
    )

    # args = parser.parse_args()
    # run_bacdive_pipeline(
    #     raw_input_dir=args.raw_input_dir,
    #     struct_dir=args.struct_dir,
    #     output_dir=args.output_dir,
    #     ref_path=args.ref_path,
    #     csn_path=args.csn_path
    # )

    run_bacdive_pipeline(
        raw_input_dir='../../sample_data/01_bacdive/raw_data',
        struct_dir='../../sample_data/01_bacdive/config/structure',
        output_dir='../../result/single_source/DB_bacdive',
        ref_path='../../sample_data/00_reference/reference.tsv',
        csn_path='../../sample_data/00_reference/csn.tsv'
    )


if __name__ == "__main__":
    main()
